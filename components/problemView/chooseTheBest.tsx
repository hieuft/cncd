...
export default function ChooseTheBest({
  problemIndex,
  statement,
  optionsList,
  showAnswer,
  ...
  useEffect(() => {
    function updateUserAnswer(i: number) {
      const userAnswer = document.getElementById(
        "user-answer" + problemIndex.toString(),
      );
      if (userAnswer) userAnswer.innerHTML = i.toString();
    }
        ...
        {optionsList.map((item) => (
          ...
          >
            ...
            <input
              ...
              type="radio"
              name={"choose the best answer " + problemIndex.toString()}
          ...
          {optionsList.map((item) => (
            ...
            >
              ...
              <input
                ...
                type="radio"
                name={"choose the best answer usr" + problemIndex.toString()}
                className={
                ...
                checked={item.key.toString() == usrAnswer}
                disabled={item.key.toString() != usrAnswer}
                readOnly
              />
              <span className="absolute top-2/4 -translate-y-1/2 left-2 w-10 h-10 bg-gray-200 rounded-full"></span>
            </label>
          ...
          {optionsList.map((item) => (
            ...
            >
              ...
              <input
                ...
                type="radio"
                name={"choose the best answer ans" + problemIndex.toString()}
                className="absolute opacity-0 cursor-pointer [&:checked+span]:!bg-blue-500"
                checked={item.key.toString() == answer}
                disabled={item.key.toString() != answer}
                readOnly
              />
              <span className="absolute top-2/4 -translate-y-1/2 left-2 w-10 h-10 bg-gray-200 rounded-full"></span>
            </label>
